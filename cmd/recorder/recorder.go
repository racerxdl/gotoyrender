...
func run() {
	...
	fps := 60
	seconds := 2
	ve := MakeVideoEncoder("test.avi", width, height, fps)

	// You need to create a window to initialize OpenGL Context
	...
	for i := 0; i < fps*seconds; i++ { // 10 seconds
		tr.Render()
		...
		s := tr.ScreenShot()
		ve.PutFrame(s)
		tr.StepMovieTime(1.0 / float32(fps))
