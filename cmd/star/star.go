...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	vec3 orange			= vec3( 0.05, 0.25, 0.49 );
	vec3 orangeRed		= vec3( 0.04, 0.64, 0.8 );
	
	orangeRed.x = sin(iTime)*1.0;
	orangeRed.y = sin(freqs[1])*sin(iTime);
	orangeRed.z = cos(iTime)*1.0;
	...
	float starGlow	= min( max( 0.5 - dist * ( 1.0 - brightness ), 0.0 ), 1.0 );
...
func loadImage(filename string) *image.NRGBA {
	...
	oimg := image.NewNRGBA(img.Bounds())
	draw.Draw(oimg, img.Bounds(), img, img.Bounds().Min, draw.Over)
